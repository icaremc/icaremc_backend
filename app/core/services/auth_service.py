...
async def phone_taken(db: AsyncSession, phone: str, role: str | None = None) -> bool:
    ...
    q = select(User).where(User.phone == phone, User.deleted_at.is_(None))
    if role:
...
async def request_signup_otp(db: AsyncSession, phone: str, purpose: str = "signup") -> dict:
    if await phone_taken(db, phone, role="patient" if purpose == "signup" else "doctor"):
...
async def register_patient(
    ...
    *,
    ...
    if await phone_taken(db, phone):
        return {"ok": False, "detail": "Phone already registered"}

    user = User(phone=phone, password_hash=hash_password(password), role="patient")
    db.add(user)
    await db.flush()
    ...
    return {"ok": True, "access_token": token, "token_type": "bearer", "user_id": str(user.id)}
...
async def register_doctor(
    ...
    *,
    ...
    user = User(phone=phone, password_hash=hash_password(password), role="doctor")
    db.add(user)
    await db.flush()
    code = _referral_code()
...
async def login(db: AsyncSession, *, phone: str, password: str, expected_role: str | None = None) -> dict:
    ...
    return {"ok": True, "access_token": token, "token_type": "bearer", "user_id": str(user.id), "role": user.role}
...
async def admin_login(db: AsyncSession, *, email: str, password: str) -> dict:
    ...
    return {"ok": True, "access_token": token, "token_type": "bearer", "user_id": str(user.id), "admin_role": admin.admin_role}
...
async def ensure_admin(
    ...
    *,
    ...
    user = User(phone=phone, email=email.lower(), password_hash=hash_password(password), role="admin")
    db.add(user)
    await db.flush()
